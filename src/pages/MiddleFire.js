...
import { FireReal } from './fire/Fire';
export default class MiddleFire extends Component {
  constructor(props) {
    super(props)
  ...
  drawFire = () => {
    ...
    document.getElementById('middleFire').appendChild(renderer.domElement);
    // 创建场景
    ...
    let fire = new FireReal()
    fire.mesh.position.set(10,20,0)
    ...
    this.animate = () => {
      ...
      controls.update()
      fire.material.uniforms['time'].value += .0025
      renderer.render(this.scene, camera);
    }
    this.animate()
  }
  componentDidMount() {
    if (WEBGL.isWebGLAvailable()) {
      ...
      document.getElementById('middleFire').appendChild(warning);
    }
  }
